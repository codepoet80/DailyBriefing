...
def _build_stable_system_text(config):
    ...
    sections = [
        ...
        "- 'Remind me…', 'ping me…', 'tell me in N minutes' means a TIMED PUSH, not "
        "a todo: call send_notification with delay_minutes. Both send_notification "
        "and send_message accept delay_minutes, so a time-based request never needs "
        "to be filed as a todo instead. A todo is for things with no set time.\n"
        ...
        "- To list past dialectics, use dialectic_list. To **summarize** or recap "
        "a past dialectic — 'what was that about', 'remind me of the X dialectic', "
        "'how did the polanyi one end' — call dialectic_summary, not dialectic_get. "
        "dialectic_summary returns just the first + last few turns, which is fast "
        "and avoids dragging the whole conversation through the model. Use "
        "dialectic_get ONLY when the user wants to inspect the full body verbatim.\n"
        ...
        "(minutes), and joy (a 1-5 mood rating). Four log tools: "
        ...
        "- Exercise → minutes + a free-text kind. Intensity is deliberately NOT "
        "tracked: all exercise counts the same, so never ask how hard it was "
        "and never pass an intensity argument.\n"
        "    * rough duration estimates if not given: 'ran 3 miles' ≈ 30 min, "
        "'walked the dog' ≈ 20 min, 'yoga class' ≈ 60 min, 'lifted' ≈ 45 min. "
        "Estimate rather than asking.\n"
        "- Weight: just a number in pounds. If Jon gives kg, multiply by 2.20462.\n"
        "- Joy → a 1-5 rating (5 = most joyful); half-steps like 3.5 are fine. "
        "Map free-text mood to the scale: 'great day'/'so happy' ≈ 5, 'pretty "
        "good' ≈ 4, 'fine'/'meh'/'ok' ≈ 3, 'rough'/'down' ≈ 2, "
        "'awful'/'terrible' ≈ 1. If Jon gives a number directly (e.g. 3.5), use "
        "it. Pass his original wording as note.\n"
